...
import { Heart, Clock, Shield, ArrowRight, Ticket, Users, Star } from 'lucide-react';
...
const HomePage = () => {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: '#f8faff', minHeight: '100vh' }}>
      ...
      <section style={{ background: 'linear-gradient(135deg, #0f2d5e 0%, #1e4d9a 40%, #2563eb 100%)', padding: '100px 1.5rem 120px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center', position: 'relative' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '50px', padding: '8px 20px', marginBottom: '28px', color: '#93c5fd', fontSize: '0.85rem', fontWeight: 600, animation: 'fadeUp 0.6s ease forwards' }}>
          ...
          <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: 'clamp(2.8rem, 7vw, 5rem)', fontWeight: 900, color: 'white', lineHeight: 1.1, marginBottom: '24px', animation: 'fadeUp 0.6s ease 0.1s both' }}>
            Support Our Cause{' '}<span style={{ color: '#fb923c', display: 'inline-block', animation: 'float 3s ease-in-out infinite' }}>❤️</span>
          </h1>
          <p style={{ fontSize: '1.2rem', color: '#bfdbfe', lineHeight: 1.7, marginBottom: '40px', fontWeight: 300, animation: 'fadeUp 0.6s ease 0.2s both' }}>
            Every ticket you buy directly funds lives. Join our raffle, win amazing prizes,<br />and make a real difference in your community.
          </p>
          <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap', animation: 'fadeUp 0.6s ease 0.3s both' }}>
            <a href="#events" className="cta-btn" style={{ padding: '16px 36px', borderRadius: '14px', background: 'linear-gradient(135deg, #f97316, #ea580c)', color: 'white', fontWeight: 700, fontSize: '1.05rem', textDecoration: 'none', boxShadow: '0 8px 24px rgba(249,115,22,0.3)', display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
              <Ticket size={20} /> Get Your Ticket
            </a>
            <a href="#trust" style={{ padding: '16px 36px', borderRadius: '14px', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', color: 'white', fontWeight: 600, fontSize: '1.05rem', textDecoration: 'none', backdropFilter: 'blur(8px)', display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
      ...
      <section id="events" style={{ maxWidth: '1200px', margin: '0 auto', padding: '80px 1.5rem' }}>
        <div style={{ textAlign: 'center', marginBottom: '56px' }}>
          <span style={{ background: '#eff6ff', color: '#2563eb', padding: '6px 16px', borderRadius: '50px', fontSize: '0.8rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Active Raffles</span>
          <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 700, color: '#0f2d5e', marginTop: '16px', marginBottom: '12px' }}>Choose Your Cause</h2>
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '24px' }}>
            {events.map((event) => (
              <div key={event.id} className="event-card" style={{ background: 'white', borderRadius: '20px', boxShadow: '0 4px 20px rgba(15,45,94,0.08)', overflow: 'hidden', border: '1px solid rgba(226,232,240,0.6)' }}>
                <div style={{ background: 'linear-gradient(135deg, #1e4d9a, #2563eb)', padding: '28px 24px 24px', position: 'relative' }}>
                  <div style={{ position: 'absolute', top: 16, right: 16, background: 'rgba(255,255,255,0.15)', borderRadius: '50px', padding: '4px 12px', fontSize: '0.75rem', color: 'white', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '5px' }}>
                    <Clock size={12} /> {formatDeadline(event.deadline)}
                  </div>
                  <h3 style={{ fontFamily: "'Fraunces', serif", fontSize: '1.4rem', color: 'white', fontWeight: 700, margin: 0 }}>{event.title}</h3>
                  {event.description && <p style={{ color: '#bfdbfe', fontSize: '0.875rem', marginTop: '8px', lineHeight: 1.5 }}>{event.description.substring(0, 100)}{event.description.length > 100 ? '...' : ''}</p>}
                </div>
                <div style={{ padding: '24px' }}>
                  ...
                  <Link to={`/buy/${event.id}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', width: '100%', padding: '14px', background: 'linear-gradient(135deg, #f97316, #ea580c)', color: 'white', borderRadius: '12px', fontWeight: 700, textDecoration: 'none', fontSize: '0.95rem', boxShadow: '0 4px 16px rgba(249,115,22,0.25)', transition: 'all 0.2s' }}>
      ...
      <section id="trust" style={{ background: 'linear-gradient(135deg, #0f2d5e, #1e4d9a)', padding: '80px 1.5rem' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto', textAlign: 'center' }}>
          <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: 'clamp(2rem, 4vw, 2.8rem)', color: 'white', fontWeight: 700, marginBottom: '16px' }}>Why Trust CharityLot?</h2>
          <p style={{ color: '#93c5fd', fontSize: '1.05rem', marginBottom: '56px' }}>100% of proceeds go directly to charity. No hidden fees. No middlemen.</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '24px' }}>
            {[
              ...
              <div key={i} style={{ background: 'rgba(255,255,255,0.08)', backdropFilter: 'blur(8px)', borderRadius: '16px', padding: '32px 24px', border: '1px solid rgba(255,255,255,0.12)' }}>
          ...
          <div style={{ marginTop: '56px', color: '#93c5fd' }}>
            ...
            <a href="mailto:hello@charitylot.app" style={{ color: '#60a5fa', fontWeight: 700, fontSize: '1.1rem' }}>hello@charitylot.app</a>
          </div>
        </div>
      </section>
